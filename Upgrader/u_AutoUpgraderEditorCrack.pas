...
unit u_AutoUpgraderEditorCrack;
...
uses
  ...
  Registry,
  auAutoUpgrader, auExtAssociation, ImgList;

type
  TAutoUpgraderEditorCrack = class(TForm)
    GroupBox1: TGroupBox;
    Label1: TLabel;
    Label2: TLabel;
    DateEdit: TEdit;
  ...
  private
    FAutoUpgrader: TauAutoUpgrader;
    procedure AddURLToList(URL: String);
    procedure SetAutoUpgrader(const Value: TauAutoUpgrader);
  public
    Property AutoUpgrader: TauAutoUpgrader read FAutoUpgrader write SetAutoUpgrader;
  end;
...
// ---------------------------------------------------
procedure TAutoUpgraderEditorCrack.ExportBtnClick(Sender: TObject);
var
  I: Integer;
  ListItem: TListItem;
  TF: TextFile;
...
procedure TAutoUpgraderEditorCrack.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TAutoUpgraderEditorCrack.AddURLToList(URL: String);
var
  ListItem: TListItem;
begin
...
procedure TAutoUpgraderEditorCrack.OKBtnClick(Sender: TObject);
var
  I: Integer;
begin
...
procedure TAutoUpgraderEditorCrack.CancelBtnClick(Sender: TObject);
begin
  Close;
end;

procedure TAutoUpgraderEditorCrack.TestBtnClick(Sender: TObject);
begin
  ShowUpgradeBox(Memo1.Text);
end;

procedure TAutoUpgraderEditorCrack.ByDateRadioClick(Sender: TObject);
begin
  if ByDateRadio.Checked then
   begin
...
procedure TAutoUpgraderEditorCrack.DateEditExit(Sender: TObject);
var
  I: Integer;
  St: String;
  Valid: Boolean;
...
procedure TAutoUpgraderEditorCrack.ListViewChange(Sender: TObject;
...
procedure TAutoUpgraderEditorCrack.AddBtnClick(Sender: TObject);
begin
...
procedure TAutoUpgraderEditorCrack.RenameItemClick(Sender: TObject);
begin
  ListView.Selected.EditCaption;
end;

procedure TAutoUpgraderEditorCrack.SetAutoUpgrader(const Value: TauAutoUpgrader);
...
begin
  FAutoUpgrader := Value;
  if FAutoUpgrader <> Nil then
  begin
    DataToEditor;
  end;
end;

procedure TAutoUpgraderEditorCrack.DeleteBtnClick(Sender: TObject);
begin
...
procedure TAutoUpgraderEditorCrack.CheckURLBtnClick(Sender: TObject);
begin
  if ListView.Selected <> nil then
    OpenURL(ListView.Selected.Caption, True);
end;

procedure TAutoUpgraderEditorCrack.ListViewDblClick(Sender: TObject);
begin
  if ListView.Selected <> nil then
   ListView.Selected.EditCaption
end;
